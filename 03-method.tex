...
Because our attributes occupy 80 bytes of memory, we use quantization to represent the values in the grid.
...
In principle, it would be possible to store all voxel attributes in brick grid, though the performance gain would be small and modifying brick grid to support this would exceed the available time.
...
On average it takes $\SI{0.14}{\s}$ to filter a model and the resulting grids of all models occupy 940MB of disk space, where the finest \acsp{lod} are responsible for 65\% of this data.
...
Therefore, we exported the bounding box sizes of the volumes during filtering.
...
For fast convergence we employ next event estimation and we use importance sampling of the scattering functions and the environment map.
