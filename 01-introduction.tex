...
In the paper \citetitle{meng_multi_scale_modeling_and_rendering_of_granular_materials} \citeauthor{meng_multi_scale_modeling_and_rendering_of_granular_materials} use volumetric representations of grains for efficient path tracing \cite{meng_multi_scale_modeling_and_rendering_of_granular_materials}.
...
\citetitle{sggx} by \citeauthor{sggx} is another paper that uses the microflake framework \cite{sggx}.
...
\citeauthor{hoppe_simplification} follow this approach in the paper \citetitle{hoppe_simplification} \cite{hoppe_simplification}.
...
The authors of \citetitle{peng_simplification} use edge collapse in a \ac{lod} context \cite{peng_simplification}.
...
The paper \citetitle{CATMULL1978350} introduced an algorithm that is now widely known as Catmull-Clark subdivision \cite{CATMULL1978350}.
...
The early work \citetitle{moreton_tessellation} by \citeauthor{moreton_tessellation} compares the De Casteljau and the forward differencing algorithm for evaluating tensor product surfaces \cite{moreton_tessellation}.
